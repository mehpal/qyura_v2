...
class DiagonSticDocCounsultaion_model extends CI_Model {

    public function __construct() {
        parent::__construct();
    }

    public function getConsultantList($notIn, $diagonsticUserId, $specialityId, $search) {
...
CASE 
 ...
 AS `rating`')
                ...
                ->where(array('doctors_deleted' => 0, 'usersRoles_roleId' => ROLE_DOCTORE, 'usersRoles_parentId' => $diagonsticUserId, 'qyura_specialities.specialities_id' => $specialityId,))
...
//        echo $this->db->last_query(); exit();
    ...
    public function getDocTimeSlot($diagonsticId, $doctorUserId) {
        ...
        if (!empty($response)) {
            foreach ($response as $row) {
                ...
                $finalResult[$day] = $this->getDoctorSession($row->id, $diagonsticId);
            }
            return $finalResult;
        } else {
    ...
    public function getDoctorSession($id, $diagonsticId) {
    ...
    public function getDiagnoTimeSlot($diagonsticId) {
