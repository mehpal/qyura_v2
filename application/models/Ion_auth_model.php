...
//exit;
    ...
    public function users($groups = NULL) {
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            $this->db->select(array(
                $this->tables['users'] . '.*',
                $this->tables['users'] . '.users_id as user_id'
            ));
        }
    ...
    public function set_session($user) {
        ...
        if ($user->usersRoles_roleId == "4") {
            $session_data = array(
                'ses_doc_email' => $user->users_email,
                'ses_doc_roleid' => $user->usersRoles_roleId,
                'ses_doc_userid' => $user->users_id,
                'ses_doc_old_last_login' => $user->last_login
            );
        } else if ($user->usersRoles_roleId == "1" || $user->usersRoles_roleId == "3") {
                $session_data = array(
                    'ses_mi_email' => $user->users_email,
                    'ses_mi_roleid' => $user->usersRoles_roleId,
                    'ses_mi_userid' => $user->users_id,
                    'ses_mi_old_last_login' => $user->last_login
                );
            }
        if ($user->usersRoles_roleId == "7") {
            $session_data = array(
                'ses_sa_email' => $user->users_email,
                'ses_sa_roleid' => $user->usersRoles_roleId,
                'ses_sa_userid' => $user->users_id,
                'ses_sa_old_last_login' => $user->last_login
            );
        }
