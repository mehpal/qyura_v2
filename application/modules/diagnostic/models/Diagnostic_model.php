...
class Diagnostic_model extends CI_Model {
    ...
    function getDiagnosticdetail($diagnoId) {
        ...
        if (!empty($rows)) {
            ...
            foreach ($allCountry as $key => $val) {
                $selected = '';
                if ($val->country_id == $rows->diagnostic_countryId)
            ...
            foreach ($allState as $key => $val) {
                $selected = '';
                if ($val->state_id == $rows->diagnostic_stateId)
                    $selected = 'selected="selected"';
            ...
            foreach ($allCity as $key => $val) {
                $selected = '';
                if ($val->city_id == $rows->diagnostic_cityId)
                    $selected = 'selected="selected"';
    ...
    function fetchDiagnosticDoctorDataTables($diagonsticUserId) {
        ...
        $timeUrl = site_url() . '/diagnostic/detailDiagnostic/$2/doctor/$1/timeSlot';
        ...
        $this->datatables->add_column('view', '<a class="btn btn-info waves-effect waves-light m-b-5 applist-btn" href=' . $doctorUrl . '>Edit Detail</a><a class="btn btn-info waves-effect waves-light m-b-5 applist-btn" href=' . $timeUrl . '>Edit Time Slot</a>', 'id, diagnostic_id');
    ...
    function getDocTimeOnDay($where) {
        $con = array('qyura_docTimeTable.docTimeTable_deleted' => 0, 'qyura_docTimeDay.docTimeDay_deleted' => 0);
