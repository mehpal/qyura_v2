...
//Diagnostic Detail
    public function getDetail($qtnId) {
        ...
        $this->db->select("qyura_quotations.quotation_timeSlotId as timeSlotId,qyura_quotations.quotation_MiId,qyura_quotations.quotation_id, qyura_quotations.quotation_dateTime as dateTime, (CASE WHEN(diagnostic_usersId is not null) THEN diagnostic_mblNo WHEN(hospital_usersId is not null) THEN hospital_mblNo END) as MImblNo, (CASE WHEN(diagnostic_usersId is not null) THEN diagnostic_name WHEN(hospital_usersId is not null) THEN hospital_name END) as MIname, (CASE WHEN(diagnostic_usersId is not null) THEN diagnostic_hmsId WHEN(hospital_usersId is not null) THEN hospital_hmsId END) as hmsId, qyura_diagnosticsCat.diagnosticsCat_catName AS diagCatName, qyura_quotationBooking.quotationBooking_orderId AS orderId, qyura_quotationBooking.quotationBooking_bookStatus as bookStatus, qyura_quotationBooking.quotationBooking_orderId AS orderId,CASE qyura_quotations.quotation_qtStatus WHEN '11' THEN 'Pending' WHEN '12' THEN 'Confirmed' WHEN '13' THEN 'Canceled'  WHEN '19' THEN 'Expired' WHEN '14' THEN 'Completed' ELSE NULL END AS bookingStatus,qyura_quotations.quotation_qtStatus, CASE qyura_quotations.	quotation_payStatus WHEN '16' THEN 'Paid' WHEN '15' THEN 'Unpaid'ELSE NULL END AS paymentStatus, transactionInfo.payment_method AS paymentMethod,(CASE WHEN(diagnostic_usersId is not null) THEN diagnostic_img WHEN(hospital_usersId is not null) THEN hospital_img END) as MIimg,(CASE WHEN(diagnostic_usersId is not null) THEN 'diagnostic' WHEN(hospital_usersId is not null) THEN 'hospital' END) as type,CASE when qyura_doctorAppointment.doctorAppointment_payMode=17 THEN 'CASH' when qyura_doctorAppointment.doctorAppointment_payMode=20 THEN 'Credit Card' when qyura_doctorAppointment.doctorAppointment_payMode=21 THEN 'Net banking' when qyura_doctorAppointment.doctorAppointment_payMode=22 THEN 'Debit Card' when qyura_doctorAppointment.doctorAppointment_payMode=23 THEN 'Cash Card' when qyura_doctorAppointment.doctorAppointment_payMode=24 THEN 'Mobile Payment'ELSE 'TEST AVENUE' end as paymode,qyura_quotations.quotations_finalTime as finalTime");
