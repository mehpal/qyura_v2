...
class Appointment_model extends Common_model {
    ...
    public function QuotationList($now, $userId) {
        ...
        $sql1 = "SELECT `qyura_quotationBooking`.`quotationBooking_reportTitle` as title,"
                    ...
                    . "(CASE WHEN(quotation_dateTime > CURRENT_TIMESTAMP ) THEN 'Upcoming' ELSE 'Completed' END) as `upcomingStatus`,"
                    . "CASE qyura_quotationBooking.quotationBooking_bookStatus WHEN '1' THEN 'pending' WHEN '2' THEN 'confirmed' WHEN '3' THEN 'Cancelled' WHEN '4' THEN 'Completed' ELSE '' END AS `bookingStatus`,"
...
AND `qyura_quotations`.`quotation_dateTime` <> 0"; 
    ...
    public function PackageAppointmentList($now, $userId) {
        ...
        $sql2 = 'SELECT `healthPackage_packageTitle` as `title`,'
                    ...
                    ."(CASE healthPkgBooking_bkStatus WHEN '1' THEN 'pending' WHEN '2' THEN 'confirmed' WHEN '3' THEN 'Cancelled' WHEN '4' THEN 'Completed' ELSE ''  END) as `bookingStatus`,"
...
LEFT JOIN `qyura_doctorAvailabilitySession` ON `qyura_doctorAvailabilitySession`.`doctorAvailabilitySession_id`=`qyura_doctorAppointment`.`doctorAppointment_finalTiming`
