...
class Reviews extends MyRest {
    ...
    function setReview_post() {
        ...
        if ($this->bf_form_validation->run($this) == FALSE) {
            ...
            $aptmntId = isset($_POST['aptmntId']) ? $this->input->post('aptmntId') : '';
            ...
            if ($fav_isCheck) {
                ...
                if($data['reviews_rating'] <= 3){
                    $data['reviews_deleted'] = 2;
                }
        ...
        if ($this->bf_form_validation->run($this) == FALSE) {
            ...
            if (isset($reviews) && $reviews != NULL) {
                foreach ($reviews as $review) {
                    ...
                    //$reviews_array['patientDetails_pLastName'] = $review->patientDetails_pLastName;
		    $path = realpath(FCPATH . 'assets/patientImages');
                    $path = $path.'/'.$review->patientDetails_patientImg;
                    if(file_exists($path))
                    $reviews_array[] = 'assets/patientImages/'.$review->patientDetails_patientImg;
                    else
                    $reviews_array[] = '';
            ...
            $colums = array('reviews_details', 'reviews_relateId', 'reviews_rating', 'users_id', 'patientDetails_patientName','patientImg');
