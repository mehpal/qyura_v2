...
WHERE hospital_deleted =0 AND qyura_hospital.status=1";
    ...
    function fetchsponserdates($where) {
        ...
        $this->db->select('Spon.sponsor_date,count(*) totbooks');
        $this->db->from('qyura_healthTipSponsor Spon');
        $this->db->where($where);
	$this->db->where('Spon.sponsor_deleted','0');
        $this->db->where('Spon.sponsor_date >='.$todate);
...
//        exit;
    ...
    function fetchsponserhealthtip($where) {
...
CASE 
WHEN (Spon.sponser_userRole = 1 ) THEN (select hospital_name from qyura_hospital hos where hos.hospital_usersId=sponsor_userId group by Spon.sponsor_id)  
WHEN (Spon.sponser_userRole = 3 ) THEN (select diagnostic_name from qyura_diagnostic diag where diag.diagnostic_usersId=sponsor_userId group by Spon.sponsor_id) 
WHEN (Spon.sponser_userRole = 4 ) THEN (select concat(doctors_fName,' ',doctors_lName) from qyura_doctors doc where doc.doctors_userId=sponsor_userId group by Spon.sponsor_id)  end as miname,
CASE 
...
CASE 
WHEN (Spon.sponser_userRole = 1 ) THEN (select hospital_name from qyura_hospital hos where hos.hospital_usersId=sponsor_userId group by group by Spon.sponsor_id)  
WHEN (Spon.sponser_userRole = 3 ) THEN (select diagnostic_name from qyura_diagnostic diag where diag.diagnostic_usersId=sponsor_userId group by group by Spon.sponsor_id) 
WHEN (Spon.sponser_userRole = 4 ) THEN (select concat(doctors_fName,' ',doctors_lName) from qyura_doctors doc where doc.doctors_userId=sponsor_userId group by Spon.sponsor_id)  end as miname,
CASE 
WHEN (Spon.sponser_userRole = 1 ) THEN 'Hospital'  
WHEN (Spon.sponser_userRole = 3 ) THEN 'Diagnostic Center' 
