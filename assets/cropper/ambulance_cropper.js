(function (factory) {
  ...
  'use strict';
  var useBlobs = false && window.URL; // `true` to use Blob instead of Data-URL
  var console = window.console || { log: function () {} };

  function CropAvatar($element) {
    this.$container = $element;

    this.$avatarView = this.$container.find('.avatar-view-doctor');
    this.$avatar = this.$avatarView.find('img');
    this.$avatarModal = this.$container.find('#avatar-modal-doctor');
    this.$pre = this.$container.find('.pre-doctor');
    
    this.$loading = this.$container.find('.loading');

    this.$uploadForm = this.$container.find('#submitFormDoctor');


    this.$avatarForm = this.$container.find('.avatar-form-doctor');
...
//      });
    ...
    readImages:function (file,currentObj) {
            // 2.1
            ...
            reader.addEventListener("load", function () {
                ...
                image.src = useBlobs ? window.URL.createObjectURL(file) : reader.result;
                // If we set the variable `useBlobs` to true:
